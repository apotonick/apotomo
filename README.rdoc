...
The widget is named +post-comments+ and jumps to the +:display+ state per default. Assuming your controller has a method +#post+ we pass the current post into the widget.
...
Look at line 2 - if encountering a +:submit+ event we invoke +#process+. This will do three steps:
